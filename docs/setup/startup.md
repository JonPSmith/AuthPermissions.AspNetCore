...
The final method will register all the services and, optionally, run some migration/seeding on startup of the ASP.NET Core. There are three options:

### 1. `SetupAspNetCorePart` extension method

This assumes that the AuthP' database has already been created/migrated. This means it will work for ASP.NET Core deployments that runs multiple instances of the application (known as _scale out_ in Azure).

If you have included Bulk Load extension methods in your configuraton you must the `SetupAspNetCorePart` `addRolesUsersOnStartup` to 'true' (defaults to `false`) for the Bulk Load data is added to the database. _NOTE: This will not work if the deployments that runs multiple instances of the application._
...
_NOTE: This will not work if the deployments that runs multiple instances of the application._

### 3. `SetupForUnitTestingAsync` extension method

If you want to unit test (also known as integration test) your application while using the AuthP features you should use the `SetupForUnitTestingAsync` extension method. This returns the built `ServiceProvider`, which contains all the AuthP services.
...
1. Build the `ServiceProvider` which contains all the AuthP's services.
